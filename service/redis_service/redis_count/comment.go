...
func setDirtComment(t CacheType, commentID uint, increase bool) {
	...
	// 用Lua原子完成HINCRBY+SADD;计数键不设置TTL,避免未同步增量被过期丢弃
	if err := setDirtScript.Run(ctx, global.RedisTimeCache,
		[]string{string(t), DirtyCommentSetKey},
		strconv.Itoa(int(commentID)), delta,
	).Err(); err != nil {
		logrus.Errorf("更新评论计数缓存失败, key: %s, id: %d, err: %v", t, commentID, err)
	}
}
