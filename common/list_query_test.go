...
func TestListQuery(t *testing.T) {
	...
	t.Run("排序测试-非白名单Order被丢弃", func(t *testing.T) {
		options := Options{
		...
		//非法排序被丢弃,回退到默认的 id DESC
		if list[0].Name != "Eve" {
			t.Errorf("非法Order未被拦截, 第一个应该是 Eve, 实际是 %v", list[0].Name)
		}
	})

	t.Run("排序测试-未配置白名单时忽略Order", func(t *testing.T) {
		options := Options{
			PageInfo: PageInfo{Page: 1, Limit: 10, Order: "age asc"},
		}
		list, _, _, err := ListQuery(TestModel{}, options)
		...
		if list[0].Name != "Eve" {
			t.Errorf("未配置白名单时Order应被忽略, 第一个应该是 Eve, 实际是 %v", list[0].Name)
		}
	})
