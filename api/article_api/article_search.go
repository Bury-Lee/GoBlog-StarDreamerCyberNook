...
func (ArticleApi) ArticleSearchView(c *gin.Context) {
	...
	// ES不可用时，使用数据库全文搜索表降级
	if global.ES == nil {
		// 降级搜索,搜索表做关键词匹配,关联文章表做状态/标签过滤与排序
		baseQuery := global.DB.Table("article_search_models AS s").
			Joins("JOIN article_models AS a ON a.id = s.article_id").
			Where("a.status = ?", models.StatusPublished)
		if req.Key != "" {
			like := "%" + req.Key + "%"
			baseQuery = baseQuery.Where("(s.title LIKE ? OR s.abstract LIKE ?)", like, like)
		}
		if req.Tag != "" {
			//标签以JSON数组存储,匹配 "tag" 整体,避免子串误命中
			baseQuery = baseQuery.Where("a.tag_list LIKE ?", "%\""+req.Tag+"\"%")
		}

		var total int64
		if err := baseQuery.Count(&total).Error; err != nil {
			logrus.Errorf("降级搜索统计失败 %s", err)
		...
		// 排序:0最新发布 1猜你喜欢(降级为最新) 2最多回复 3最多点赞 4最多收藏
		orderColumns := map[int8]string{
			0: "created_at",
			1: "created_at",
			2: "comment_count",
			3: "digg_count",
		...
		orderColumn := orderColumns[req.Type]
		if orderColumn == "" {
		...
		var hits []searchHit
		if err := baseQuery.Select("s.article_id, s.title, s.abstract").
		...
		for _, hit := range hits {
			articleIDList = append(articleIDList, hit.ArticleID)
			searchArticleMap[hit.ArticleID] = ArticleBaseInfo{
				ID:       hit.ArticleID,
				Title:    hit.Title,
