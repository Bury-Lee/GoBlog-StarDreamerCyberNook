...
func (MessageApi) SiteMessageConfUpdateView(c *gin.Context) {
	...
	//使用请求DTO绑定,模型的字段没有u标签,直接用模型会导致更新map为空
	...
	userConfMap := utils_other.StructToMap(&req, "u")
	if len(userConfMap) == 0 {
		response.OkWithMsg("没有需要更新的配置", c)
		return
	}
	if err := global.DB.Model(&models.UserMessageConfModel{}).
		Where("user_id = ?", claim.UserID).
		Updates(userConfMap).Error; err != nil {
