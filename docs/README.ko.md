...
> 대응 프론트엔드 저장소: `https://github.com/azurekiln2333/azurekiln-goblog-web`
...
> ⚠️ **주의**: 프로젝트 본체는 데이터베이스 읽기/쓰기 분리를 지원하지만, **데이터베이스 간 데이터 동기화는 제공하지 않습니다**. 저장소에는 동기화용 종속성이나 도구도 포함되어 있지 않습니다 (기존 Canal / PGSync 솔루션은 제거됨). 교차 데이터베이스 또는 교차 저장소 동기화가 필요하면 프로젝트 외부에서 직접 구현하십시오.
...
> 💡 **팁**: `init/MySQL/docker-compose.yml`에서는 MySQL `5.7`을 사용합니다. 호환성 문제를 피하려면 동일한 버전을 사용하는 것이 좋습니다.
...
- `dbWrite` / `dbRead` (쓰기 DB 목록 / 읽기 DB 목록)
...
system:
  ...
  port: 8080
  env: dev
  run_mode: debug # Gin 실행 모드: debug 또는 release
...
dbWrite: # 쓰기 데이터베이스 목록 (최소 1개 필요)
  - user: root
    ...
    db_name: db
    sql_name: mysql
  # - user: root # 이 형식으로 쓰기 데이터베이스를 추가할 수 있습니다
  #   password: root
  #   host: 127.0.0.1
  #   port: 3306
  #   db_name: db
...
dbRead: # 읽기 데이터베이스 목록 (비어 있으면 읽기도 쓰기 DB 사용)
  # - user: root
  #   password: root
  #   host: 127.0.0.1
  #   port: 3306
...
프로젝트 본체는 데이터베이스 읽기/쓰기 분리를 지원하지만, **데이터베이스 간 데이터 동기화는 제공하지 않습니다**. 저장소에는 동기화용 종속성이나 도구도 포함되어 있지 않습니다 (기존 Canal / PGSync 솔루션은 제거됨).

- 쓰기 DB와 읽기 DB 간 데이터 정합성은 프로젝트 외부 운영(예: MySQL 복제)으로 보장하십시오
- 다른 저장소(예: Elasticsearch)로의 동기화가 필요하면 프로젝트 외부에서 직접 구현하십시오
...
| 문제 | 해결책 |
|------|--------|
| **MySQL 연결 실패** | `setting.yaml`의 계정, 비밀번호, 포트가 실제 MySQL과 일치하는지 확인 |
