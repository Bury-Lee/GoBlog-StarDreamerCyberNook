...
> ⚠️ **注意**：プロジェクト本体はデータベースの読み書き分離をサポートしていますが、**データベース間のデータ同期は提供していません**。リポジトリには同期用の依存やツールも含まれていません（旧 Canal / PGSync ソリューションは削除済み）。クロスデータベースやクロスストアの同期が必要な場合は、プロジェクト外で独自に実装してください。
...
| コンポーネント | バージョン要件 | 備考 |
|----------|------------|------|
| Go | 1.26.6 | プロジェクトの`go.mod`バージョンに従う |
...
> 💡 **ヒント**：`init/MySQL/docker-compose.yml`ではMySQL `5.7`を使用しています。互換性の問題を避けるため、同じバージョンを使用することを推奨します。
### 2️⃣ プロジェクトをコンパイルする
...
- `dbWrite` / `dbRead`（書き込みDB一覧 / 読み取りDB一覧）
...
system:
  ...
  port: 8080
  env: dev
  run_mode: debug # Gin実行モード：debugまたはrelease
...
dbWrite: # 書き込みデータベース一覧（最低1つ必要）
  - user: root
    ...
    db_name: db
    sql_name: mysql
  # - user: root # この形式で書き込みデータベースを追加できます
  #   password: root
  #   host: 127.0.0.1
  #   port: 3306
  #   db_name: db
...
dbRead: # 読み取りデータベース一覧（空の場合、読み取りも書き込みDBを使用）
  # - user: root
  #   password: root
  #   host: 127.0.0.1
  #   port: 3306
...
プロジェクト本体はデータベースの読み書き分離をサポートしていますが、**データベース間のデータ同期は提供していません**。リポジトリには同期用の依存やツールも含まれていません（旧 Canal / PGSync ソリューションは削除済み）。

- 書き込みDBと読み取りDB間のデータ整合性は、プロジェクト外の運用（MySQL のレプリケーションなど）で担保してください
- 他のストア（Elasticsearch など）への同期が必要な場合は、プロジェクト外で独自に実装してください
...
| 問題 | 解決策 |
|------|--------|
| **MySQL接続失敗** | `setting.yaml`のアカウント、パスワード、ポートが実際のMySQLと一致しているか確認 |
